...
Router.post("/signup", (req, res) => {
  User.findOne({ Email: req.body.Email }).exec(async (error, user) => {
    ...
    const { Name, Moto, Phone, Age, Email, Password } = req.body;
    ...
    _user.save((error, data) => {
      ...
      if (data) {
        const token = jwt.sign({ _id: data._id }, "MERNSECRET", {
          expiresIn: "3h",
        });
        const _id = data._id;
  ...
  User.findOne({ Email: req.body.Email }).exec(async (error, user) => {
    ...
    if (user) {
      if (user.authenticate(req.body.Password)) {
        ...
        const { _id, Name, Moto, Phone, Age, Email, pofilePicture } = user;
        res.status(200).json({
          token,
          user: {
            _id,
      ...
      return res
        .status(400)
        .json({ message: "Invalid Credential - Server Timed Out" });
    }
  });
});
...
Router.post("/unfollow", (req, res) => {
  ...
  Follow.find({ To: To, From: From }).deleteMany((error, data) => {
    if (error) {
      return res.status(400).json({
        message: error,
      });
        ...
        User.findOne({ _id: item.From })
          .then((follower) => {
            ...
            tmp = {
              Name: follower.Name,
              _id: follower._id,
              pofilePicture: follower.pofilePicture,
            };
            console.log(tmp);
            array.push(tmp);
            ss++;
...
Router.post("/updatedata", (req, res) => {
  ...
  User.updateOne(
    ...
    { pofilePicture: req.body.url, Name: req.body.Name, Moto: req.body.Moto }
  )
    .then((user) => {
      return res.status(200).json(user);
    })
...
Router.post("/deletewish", (req, res) => {
  const { Pic, Title, Heading, Subject, Priority } = req.body;
